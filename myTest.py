...
#                        cv2.imwrite("./data/"+VideoName_short+"_s"+str(index)+".jpg", myFrame)
#                        np.savetxt("./data/"+VideoName_short+"_"+str(index)+".txt", myFrame, fmt="%d")
			Out_frame = np.uint8(Out_frame)
			videoWriter.write(Out_frame)
                        myFrame = np_predict[0,index, :, :, 0]
                        myFrame = myFrame - np.amin(myFrame)
                        myFrame = myFrame / np.amax(myFrame)
                        myFrame = scipy.ndimage.interpolation.zoom(myFrame, tuple(np.asarray([VideoSize[1],VideoSize[0]], dtype=np.float32) / np.asarray(myFrame.shape,dtype=np.float32)), np.dtype(np.float32), mode='nearest')
                        myFrame = myFrame * 255
